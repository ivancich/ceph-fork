...
int rgw_bucket_list(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  for (int attempt = 0;
	...
	auto eiter = entrybl.cbegin();
      ...
      CLS_LOG(0, "ERIC %s: start_after_entry_key set to '%s'", __func__, escape_str(start_after_entry_key.to_string()).c_str());
  ...
  if (ret.is_truncated && name_entry_map.size() == 0) {
    ...
    CLS_LOG(0, "ERIC %s: returning RGWBucketListAdvanceRetryError\n", __func__);
