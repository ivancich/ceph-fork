...
namespace ceph {
  ...
  public:
    ...
    crimson::dmclock::ClientInfo op_class_client_info_f(const osd_op_type_t& op_type);
    ...
    inline void enqueue_strict(Client cl,
      ...
      queue.enqueue_strict(client_info_mgr.osd_op_type(item.second),
			   priority,
			   item);
    }
    ...
    inline void enqueue_strict_front(Client cl,
				     unsigned priority,
				     Request item) override final {
      queue.enqueue_strict_front(client_info_mgr.osd_op_type(item.second),
				 priority,
				 item);
    }
    ...
    inline void enqueue(Client cl,
			...
			unsigned cost,
			Request item) override final {
      queue.enqueue(client_info_mgr.osd_op_type(item.second),
		    priority,
		    cost,
		    item);
    }
    ...
    inline void enqueue_front(Client cl,
			      ...
			      unsigned cost,
			      Request item) override final {
      queue.enqueue_front(client_info_mgr.osd_op_type(item.second),
			  priority,
			  cost,
			  item);
    }
